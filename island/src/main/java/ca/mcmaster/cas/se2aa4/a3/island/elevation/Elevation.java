...
public abstract class Elevation {
    ...
    public void neighborRing(Structs.Property tier, Structs.Property tierSub1, Structs.Property tierSub2,
            Structs.Mesh aMesh, Structs.Mesh.Builder iMesh) {
        ...
        for (int i = 0; i < aMesh.getPolygonsCount(); i++) {
            ...
            if (p.getProperties(0).getValue().equals(tierSub1.getValue())) {
                for (int j = 0; j < p.getNeighborIdxsCount(); j++) {
                    ...
                    // If the neighbor is not already a tier -2 or tier -1 polygon, set its property
                    // to tier
